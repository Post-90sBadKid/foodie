package com.wry.foodie.common.exception;
...
public class GlobalExceptionHandler {
    ...
    public Result<Object> handleThrowable(HttpServletRequest request, Throwable e) {
        log.error("执行方法异常，访问地址：{}", request.getRequestURI(), e);
...
//    }
     ...
     * <p> 这两个注解都是实现JSR-303规范，不同的是@Validated是spring的注解支持groups以及可以用在spring mvc处理器的方法级别入参验证 ，
     * @Valid是Javax提供的注解，可以支持多个bean嵌套验证。
     *
     * @param request
     * @param e
     * @return
    ...
    public Result handleJSR303Exception(HttpServletRequest request, Exception e) {
        ...
        Result result = Result.errorVerification("参数校验失败！");
